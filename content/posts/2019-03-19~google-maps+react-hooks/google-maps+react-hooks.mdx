...
Note that the `onMount` function must be [curried](https://en.wikipedia.org/wiki/Currying) since the `Map` component itself calls that function, first passing in the forwarded arguments (`onMountProps`) and only then supplying the `map` object.
